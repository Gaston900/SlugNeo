...
//***************GSC2007*******新增代码****3段****平滑斜方向切换**********        
        if(((m_current & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT)) ==(LEFT_BIT | DOWN_BIT)) &&((m_previous  & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT)) ==(RIGHT_BIT | DOWN_BIT)) )
                m_current &= ~(LEFT_BIT | RIGHT_BIT);//输入左下,右下时，纠正为左下,下等...屏蔽左右，补全下上
        if(((m_current & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT)) ==(RIGHT_BIT | DOWN_BIT))&&((m_previous  & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT)) ==(LEFT_BIT | DOWN_BIT)) )
                m_current &= ~(LEFT_BIT | RIGHT_BIT);//同上。屏蔽左右，补全下上
        if(((m_current & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT)) ==(LEFT_BIT | UP_BIT)) &&((m_previous  & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT)) ==(RIGHT_BIT | UP_BIT)) )
                m_current &= ~(LEFT_BIT | RIGHT_BIT);//同上。屏蔽左右，补全下上
        if(((m_current & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT)) ==(RIGHT_BIT | UP_BIT))&&((m_previous  & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT)) ==(LEFT_BIT | UP_BIT)) )
                m_current &= ~(LEFT_BIT | RIGHT_BIT);//同上。屏蔽左右，补全下上

        if(((m_current & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT))==(LEFT_BIT | DOWN_BIT)) &&((m_previous & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT))==(LEFT_BIT | UP_BIT)) )
                m_current &= ~(UP_BIT | DOWN_BIT);//同上。屏蔽下上，补全左右
        if(((m_current & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT))==(RIGHT_BIT | DOWN_BIT))&&((m_previous & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT))==(RIGHT_BIT | UP_BIT)) )
                m_current &= ~(UP_BIT | DOWN_BIT);//同上。屏蔽下上，补全左右
        if(((m_current & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT))==(LEFT_BIT | UP_BIT)) &&((m_previous & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT))==(LEFT_BIT | DOWN_BIT)) )
                m_current &= ~(UP_BIT | DOWN_BIT);//同上。屏蔽下上，补全左右
        if(((m_current & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT))==(RIGHT_BIT | UP_BIT))&&((m_previous & (LEFT_BIT | RIGHT_BIT | UP_BIT| DOWN_BIT))==(RIGHT_BIT | DOWN_BIT)) )
                m_current &= ~(UP_BIT | DOWN_BIT);//同上。屏蔽下上，补全左右
...
{
	std::string seqdefstring;
	std::string newstring;

	// FILE *pfile;
	// pfile = fopen("load config.txt","a");
	// fprintf(pfile,"====================================\n");

	//int getype = (int)cfg_type;

	// fprintf(pfile,"cfg_type : %d \n",getype);
	...
	for (util::xml::data_node const *portnode = parentnode->get_child("port"); portnode; portnode = portnode->get_next_sibling("port"))
	{
		...
		// fprintf(pfile,"player : %d , type : %d , ",player,type);
		...
		for (util::xml::data_node const *seqnode = portnode->get_child("newseq"); seqnode; seqnode = seqnode->get_next_sibling("newseq"))
		{
			...
			// fprintf(pfile, "seqtype : %d , seqname : %s , ",token_to_seq_type(seqnode->get_attribute_string("type", "")),seqnode->get_attribute_string("type", ""));


			if (seqtype != -1 && seqnode->get_value() != nullptr)
			{
				// fprintf(pfile, "seqnode->get_value : %s\n",seqnode->get_value());
			...
			// newstring = machine().input().seq_to_tokens(newseq[seqtype]);
			// fprintf(pfile,"new seq string %s \n",newstring.c_str());
	...
	if (cfg_type == config_type::CONTROLLER)
	{
		for (input_type_entry &entry : m_typelist)
		{
			for (input_seq_type seqtype = SEQ_TYPE_STANDARD; seqtype < SEQ_TYPE_TOTAL; ++seqtype)
			{
				...
				// seqdefstring = machine().input().seq_to_tokens(entry.defseq(seqtype));
				// fprintf(pfile,"default value : %s\n",seqdefstring.c_str());
			}
		}
	}
...
{
	//  FILE *pfile;
	//  pfile = fopen("load_default_config.txt","a");
		...
		if (entry.type() == type && entry.player() == player)
		{
			for (input_seq_type seqtype = SEQ_TYPE_STANDARD; seqtype < SEQ_TYPE_TOTAL; ++seqtype)
				if (newseq[seqtype][0] != INPUT_CODE_INVALID)
				{
					...
					// setstring = machine().input().seq_to_tokens(newseq[seqtype]);
					// defstring = machine().input().seq_to_tokens(entry.defseq(seqtype));
					// seqstring = machine().input().seq_to_tokens(entry.seq(seqtype));
					
					// fprintf(pfile,"set value 	 : %s\n",setstring.c_str());
					// fprintf(pfile,"default value : %s\n",defstring.c_str());
					// fprintf(pfile,"cur value 	 : %s\n",seqstring.c_str());
					// fprintf(pfile,"=======================================\n");
...
{
	// FILE *pfile;
	// pfile = fopen("load_game_config.txt","a");
					...
					field.mask() == mask && (field.defvalue() & mask) == (defvalue & mask))
				{
					...
					for (input_seq_type seqtype = SEQ_TYPE_STANDARD; seqtype < SEQ_TYPE_TOTAL; ++seqtype)
						if (newseq[seqtype][0] != INPUT_CODE_INVALID)
						{
							...
							//fprintf(pfile,"field.live.seq :  %s\n",newseq[seqtype]);
						}
...
{
	// FILE *pfile;
	// pfile=fopen("save_default_inouts.txt","a");
	// fprintf(pfile,"===============================\n");
	// std::string seqstring;
	// std::string seqdefstring;
			...
			if (seqtype < SEQ_TYPE_TOTAL)
			{
				...
				if (portnode != nullptr)
				{
					...
					for (input_seq_type type = SEQ_TYPE_STANDARD; type < SEQ_TYPE_TOTAL; ++type)
						...
						{
							// seqstring = machine().input().seq_to_tokens(entry.seq(type));
							// seqdefstring = machine().input().seq_to_tokens(entry.defseq(type));
							// fprintf(pfile,"default type %s : entry type %s\n",seqdefstring.c_str(),seqstring.c_str());
